...
func TestHandler_isEmulationAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		allowedAccounts map[tongo.AccountID]struct{}
		msgBoc          string
		wantAllowed     bool
		wantErr         string
	}{
		...
		{
			name:      "highload wallet - allow because the account listed in allowedAccounts",
			accountID: ton.MustParseAccountID("0:4d8137641f40829518efb4766fea041de0df89ead2fb785d19de7298c8727a79"),
			msgBoc:    "",
			allowedAccounts: map[tongo.AccountID]struct{}{
				ton.MustParseAccountID("0:4d8137641f40829518efb4766fea041de0df89ead2fb785d19de7298c8727a79"): {},
			},
			wantAllowed: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := &Handler{
				mempoolEmulationAllowedAccounts: tt.allowedAccounts,
			}

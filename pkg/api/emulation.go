...
func (h *Handler) isEmulationAllowed(account ton.AccountID, state tlb.ShardAccount, m tlb.Message) (bool, error) {
	if _, ok := h.mempoolEmulationAllowedAccounts[account]; ok {
		return true, nil
	}

...
// Encode implements json.Marshaler.
func (s *ExtraCurrencies) Encode(e *jx.Encoder) {
	e.ObjStart()
	s.encodeFields(e)
	e.ObjEnd()
}
...
// encodeFields encodes fields.
func (s *ExtraCurrencies) encodeFields(e *jx.Encoder) {
	{
		e.FieldStart("extra_currencies")
		e.ArrStart()
		for _, elem := range s.ExtraCurrencies {
			elem.Encode(e)
		}
		e.ArrEnd()
	}
...
var jsonFieldsNameOfExtraCurrencies = [1]string{
	0: "extra_currencies",
}

// Decode decodes ExtraCurrencies from json.
func (s *ExtraCurrencies) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode ExtraCurrencies to nil")
	}
	var requiredBitSet [1]uint8
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		case "extra_currencies":
			requiredBitSet[0] |= 1 << 0
			if err := func() error {
				s.ExtraCurrencies = make([]EcPreview, 0)
				if err := d.Arr(func(d *jx.Decoder) error {
					var elem EcPreview
					if err := elem.Decode(d); err != nil {
						return err
					}
					s.ExtraCurrencies = append(s.ExtraCurrencies, elem)
					return nil
				}); err != nil {
					return err
				}
				...
				return errors.Wrap(err, "decode field \"extra_currencies\"")
			}
		default:
			return d.Skip()
		}
		...
		return errors.Wrap(err, "decode ExtraCurrencies")
	}
	// Validate required fields.
	var failures []validate.FieldError
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfExtraCurrencies) {
					name = jsonFieldsNameOfExtraCurrencies[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
// MarshalJSON implements stdjson.Marshaler.
func (s *ExtraCurrencies) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *ExtraCurrencies) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
